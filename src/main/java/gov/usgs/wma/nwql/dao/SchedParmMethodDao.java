...
/**
 * Data access object for accessing nwis_sched_parm_methcd_vw in the database.
 *
 * @author kmschoep
 *
 *
 */
public class SchedParmMethodDao {
	...
	public void streamResults(OutputStream output,
		...
		try (SqlSession session = getNewSession();
			...
			session.select(MYBATIS_CORE_PACKAGE + ".doSchedParmMethodQuery",
	...
	public Integer getRowCount() {
		Integer result = null;
		try (SqlSession session = getNewSession()) {
			result = session.selectOne(MYBATIS_CORE_PACKAGE + ".doSchedParmMethodQueryCount");
		} 
		return result;
	}
