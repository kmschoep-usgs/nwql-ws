...
/**
 * Data access object for accessing nwis_spk_lot in the database.
 *
 * @author kmschoep
 *
 *
 */
public class SpikeLotDao {
	...
	public void streamResults(OutputStream output,
		...
		try (SqlSession session = getNewSession();
			...
			session.select(MYBATIS_CORE_PACKAGE + ".doSpikeLotQuery",
	...
	public Integer getRowCount() {
		Integer result = null;
		try (SqlSession session = getNewSession()) {
			result = session.selectOne(MYBATIS_CORE_PACKAGE + ".doSpikeLotQueryCount");
		} 
		return result;
	}
